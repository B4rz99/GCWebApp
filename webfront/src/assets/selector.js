...
export default function BasicSelect({ onSelectorChange }) {
    ...
    useEffect(() => {
        ...
        async function fetchDevices() {
            try {
                const response = await axios.get(`${API_URL}/api/unavailableDevices`);
    ...
    // Render the names of selected devices instead of their IDs
    const renderSelectedDeviceNames = (selected) => {
        const selectedDevices = devices.filter(device => selected.includes(device.DeviceId));
        const selectedDeviceNames = selectedDevices.map(device => device.Name);
        return selectedDeviceNames.join(', ');
    };
        ...
        <Box sx={{ minWidth: 200 }}>
            <FormControl sx={{ m: 1, width: 200 }}>
                ...
                <Select
                    ...
                    renderValue={renderSelectedDeviceNames} // Use the new function to render device names
                >
                    {devices.map((device) => (
